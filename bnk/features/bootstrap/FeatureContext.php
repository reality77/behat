...
JS;
   ...
   */
  public function iFillVisibleFieldWith($elementId, $value)
  {
    $page = $this->getSession()->getPage();

    $fields = $page->findAll('named', array('id_or_name', $elementId));
    ...
    throw new Exception("No $elementId was found");
  }
  
  /**
   * @When I fill :arg1 with the content of :arg2
   */
  public function iFillVisibleFieldWithFile($elementId, $filename)
  {
    $page = $this->getSession()->getPage();
    ...
    /** @var \Behat\Mink\Element\NodeElement $field */
    foreach ($fields as $field) {
        $field->setValue(file_get_contents($filename));
        return;
    }

    throw new Exception("No field $elementId was found");
  }

  /**
   * @When I press the button :arg1
   */
  public function iPressVisibleButton($elementValue)
  {
    $page = $this->getSession()->getPage();

    $buttons = $page->findAll('named', array('button',$elementValue));
    ...
    throw new Exception("No buton $elementValue found");
  }  
}
